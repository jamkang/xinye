...
class UserQues extends Controller
{
    ...
    public function add(){
        ...
        if( $limit != 1 && $limit != 2 && $limit != 0) return json(['error'=>'请先登录']);

        $ques = new QuestionRe;
        $data = input('post.');
        if(mb_strlen($data['contect'])<1){
            return json('内容不能为空',404);
        }
        $reg="/^1[^0][0-9]{9}$/";
        if(!preg_match($reg,$data['tele']))
        {
            return json('请填写正确的电话号码',404);
        }
